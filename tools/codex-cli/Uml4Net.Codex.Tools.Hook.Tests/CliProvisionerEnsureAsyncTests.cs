...
namespace Uml4Net.Codex.Tools.Hook.Tests
{
    ...
    public class CliProvisionerEnsureAsyncTests
    {
        ...
        public void SetUp()
        {
            this.pluginRoot = Path.Combine(Path.GetTempPath(), "uml4net-codex-hook-tests", Guid.NewGuid().ToString("N"));
        ...
        public async Task EnsureAsync_downloads_verifies_and_extracts_a_matching_release()
        {
            ...
            var assetName = $"uml4net-codex-1.0.0-{this.runtimeIdentifier}.zip";
        ...
        public async Task EnsureAsync_returns_null_when_no_asset_matches_this_platform()
        {
            ...
            {
                TagName = "tools-v1.0.0",
                Body = "uml4net-codex-1.0.0-some-other-rid.zip sha256:" + new string('a', 64),
                Assets = [new GitHubReleaseAsset { Name = "uml4net-codex-1.0.0-some-other-rid.zip", BrowserDownloadUrl = "https://example.com/x.zip" }],
            });
