...
namespace Uml4Net.Codex.Tools.Hook
{
    ...
    /// <summary>
    /// Computes where the full <c>uml4net-codex</c> CLI is cached on disk, outside the plugin's own
    /// git-managed tree so a plugin update/reinstall never disturbs it.
    /// </summary>
    public static class CacheLayout
    {
        /// <summary>
        /// The root cache directory: <c>%LOCALAPPDATA%\uml4net-codex</c> on Windows, the XDG/home
        /// equivalent elsewhere.
        /// </summary>
        public static string CacheRoot => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "uml4net-codex");
